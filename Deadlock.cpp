...
int main() {
    ...
    if (bytes <= 0) {
        cout << RED << "ERROR: File empty or unreadable.\n" << RESET;
        return 1;
    }
    buf[bytes] = '\0';
    ...
    if (tokens.size() < 3) {
        cout << RED << "ERROR: Invalid or incomplete data.\n" << RESET;
        return 1;
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < m; ++j) {
            if (!(row >> alloc[i][j])) {
                cout << RED << "ERROR: Allocation row size mismatch\n" << RESET;
                return 1;
            }
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < m; ++j) {
            if (!(row >> maxm[i][j])) {
                cout << RED << "ERROR: Max row size mismatch\n" << RESET;
                return 1;
            }
        }
    }

    {
        ...
        for (int j = 0; j < m; ++j) {
            if (!(row >> avail[j])) {
                cout << RED << "ERROR: Available row size mismatch\n" << RESET;
                return 1;
            }
        }
    }
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < m; ++j)
            if (alloc[i][j] > maxm[i][j]) {
                cout << RED << "ERROR: alloc > max detected\n" << RESET;
                return 1;
            }
    ...
    for (auto &kv : G) {
        int node = kv.first;
        if (!vis[node] && dfs(node, G, vis, instack)) {
            circularWait = true;
            break;
        }
    }
    ...
    if (circularWait)
        cout << RED << "🔴 Circular Wait Detected (Cycle Found)\n" << RESET;
    else
        cout << GREEN << "🟢 No Circular Wait\n" << RESET;
    ...
    for (int step = 0; step < n; ++step) {
        ...
        for (int p = 0; p < n; ++p) {
            ...
            bool ok = true;
            for (int r = 0; r < m; ++r)
                if (need[p][r] > avail_copy[r]) ok = false;
    ...
    for (int p : safe) {
        ...
        cout << "  Need = [ ";
        for (int r = 0; r < m; ++r) cout << need[p][r] << " ";
        cout << "]  Available = [ ";
        for (int r = 0; r < m; ++r) cout << availSim[r] << " ";
        cout << "]\n";

        sleep(1);

        cout << GREEN << "  ✔ P" << p << " completed. Releasing resources.\n" << RESET;

        for (int r = 0; r < m; ++r)
            availSim[r] += alloc[p][r];
